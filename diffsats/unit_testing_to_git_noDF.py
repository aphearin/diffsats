...
def test_orbit_UNIT_noDF(Mhost_, mass_ratio, conc_, phi0_max, 
    ...
    Vphi_DS = np.full((steps), ht_nfw.circular_velocity(Rhalo_mpc_h, Mhalo_h, conc_)
                      *0.5*km_s_to_kpc_gyr)
                    ...
                    for k in range(len(Vphi)):
                        for l in range(len(Vz0_vals)):
                            xv_in_loop = jnp.array([R[g], phi0_vals[h], z0_vals[i], 
                                                   VR0_vals[j], Vphi_DS[k], Vz0_vals[l]])
    ...
    for i in range(len(phi_DS_noDF[0:idx_range])):
        for j in range(len(phi_SG_noDF[0:idx_range])):
            np.testing.assert_allclose(phi_DS_noDF[i], phi_SG_noDF[j], rtol=1e-2, atol=1e-2)
