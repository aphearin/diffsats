#--- 
#- Functions to get the cos(theta) for varying
#- initial orbital conditions, and compute the
#- pericentric distance with respect to
#- cos(theta).

#- This is only for SatGen.
...
from diffsats import nfw as diffsats_nfw
...
def satgen_noDF_theta(Mratio, cNFW_, r0, phi0, z0, Vr0, Vphi0, Vz0):
    ...
    timesteps = np.linspace(0., 10, Nstep)[1::] # [Gyr]
    ...
    phio = oNFWnoDF.xvArray[:,1]
    zo = oNFWnoDF.xvArray[:,2]
    Vro = oNFWnoDF.xvArray[:,3]
    Vphio = oNFWnoDF.xvArray[:,4]
    Vzo = oNFWnoDF.xvArray[:,5]
...
def d_t_peri_finder(list_name, time_array):
    ...
    d_peri_vals = [minval for i, minval in enumerate(list_name)
          if ((i == 0) or (list_name[i-1] >= minval))
          and ((i == len(list_name) - 1) or (minval < list_name[i+1]))]
